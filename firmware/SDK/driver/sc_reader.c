    ...
    if (flag)
    {
        ...
        if (flag & ATR_DONE_TASK)
        {
            ...
            else
            {
                ...
                else if (!atr_decoder_get_protocol())   // T0 specific mode
                {
                    // set fd
...
__STATIC_INLINE bool get_byte(uint8_t* data)
{
    while (true)
    {
        ...
        else if (!card_in)   // card being removed, no timeout will be detected
            break;
    }
